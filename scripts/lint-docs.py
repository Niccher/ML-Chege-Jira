...
def lint_markdown_links(repo_root: Path):
    ...
    for md_file in repo_root.glob("**/*.md"):
        # Skip vendor, node_modules, git, and virtualenvs
        parts = md_file.parts
        if any(p in parts for p in ['.git', 'vendor', 'node_modules', 'venv', '.venv', '.agents', 'site']):
            continue
...
def lint_mermaid_blocks(repo_root: Path):
    ...
    for md_file in repo_root.glob("**/*.md"):
        parts = md_file.parts
        if any(p in parts for p in ['.git', 'vendor', 'node_modules', 'site', '.venv', 'venv']):
            continue
